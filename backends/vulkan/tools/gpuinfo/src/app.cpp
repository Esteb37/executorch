...
class App {
 ...
 public:
  ...
  void const_mem_bandwidth() {
    ...
    const int NTHREAD = NVEC;
    const uint local_x = NTHREAD_WARP;
    const uint global_x = (NTHREAD / local_x * local_x) * NSM * NFLUSH;

    auto bench = [&](size_t access_size) {
      const size_t nvec_access = access_size / VEC_SIZE;

      StorageBuffer in_buf(
          context(), vkapi::kFloat, access_size / sizeof(float));
      StorageBuffer out_buf(context(), vkapi::kFloat, VEC_WIDTH * NTHREAD_WARP);
      vkapi::PipelineBarrier pipeline_barrier{};
